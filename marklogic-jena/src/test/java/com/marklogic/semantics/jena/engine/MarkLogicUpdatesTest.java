...
public class MarkLogicUpdatesTest extends JenaTestBase {
	...
	public void testSparqlUpdatePermissions() {
        ...
        GraphPermissions graphPermissions = dsg.getPermissions(NodeFactory.createURI("http://example.org/gp1"));
        dsg.setSPARQLUpdatePermissions(graphPermissions.permission("semantics-peon-role", Capability.READ));

        update = new UpdateRequest();
        update.add("BASE <http://example.org/> INSERT DATA { GRAPH <gp2> { <s4882> <p132> <o12321> } }");
        UpdateAction.execute(update, dsg);

        GraphPermissions gp2 = dsg.getPermissions(NodeFactory.createURI("http://example.org/gp2"));
        assertTrue("SPARQL Update permissions assigned", gp2.get("semantics-peon-role").contains(Capability.READ) );
        ...
        gp2 = dsg.getPermissions(NodeFactory.createURI("http://example.org/gp2"));
        assertNull("SPARQL Update permissions assigned", gp2.get("semantics-peon-role") );
